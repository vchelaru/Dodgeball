    ...
    public partial class AIController
    {
        ...
        private AI2DInput.Directions WanderDirection()
        {
            ...
            if (_timeWandering < _timeToWander && _wanderDirection != AI2DInput.Directions.None)
            {
                ...
                var currentDirection = _wanderDirection;
                _wanderDirection = RemoveOffendingDirections(_wanderDirection);

                if (_wanderDirection != currentDirection) _wanderDirection = AI2DInput.Directions.None;
            ...
            else
            {
                ...
                if (_player.Y > 200) upDown = AI2DInput.Directions.Down;
                else if (_player.Y < -510) upDown = AI2DInput.Directions.Up;
        ...
        private AI2DInput.Directions DodgeDirection()
        {
            ...
            {
                //If player has gone out of bounds, start a new direction
                var currentDirection = _dodgeDirection;
                _dodgeDirection = RemoveOffendingDirections(_dodgeDirection);

                if (_dodgeDirection != currentDirection) _dodgeDirection = AI2DInput.Directions.None;
            ...
            else
            {
                ...
                //TODO:  Measure size of arena rather than hard-code it
                if (_player.Y > 200 || _player.Y < -510) upDown = AI2DInput.Directions.None;
                if (_player.X > 900 || _player.X < -900) leftRight = AI2DInput.Directions.None;
        ...
        private AI2DInput.Directions RetrieveBallDirections()
        {
            ...
            if (_player.Position.X >= -30 && _player.TeamIndex == 0) leftRight = AI2DInput.Directions.None;
            if (_player.Position.X <= 30 && _player.TeamIndex == 1) leftRight = AI2DInput.Directions.None;
        ...
        private AI2DInput.Directions RemoveOffendingDirections(AI2DInput.Directions originalDirections)
        {
            ...
            //TODO:  Measure size of arena rather than hard-code it
            if (_player.Y > 200) newDirections &= ~(AI2DInput.Directions.Up);
            else if (_player.Y < -510) newDirections &= ~(AI2DInput.Directions.Down);

            if (_player.X > 900) newDirections &= ~(AI2DInput.Directions.Right);
            else if (_player.X < -900) newDirections &= ~(AI2DInput.Directions.Left);

            //Don't let player cross center boundary
            if (_player.Position.X >= -30 && _player.TeamIndex == 0)
                newDirections &= ~(AI2DInput.Directions.Right);
            if (_player.Position.X <= 30 && _player.TeamIndex == 1)
